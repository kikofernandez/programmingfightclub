...
at Uppsala University by Dave Clarke and Kiko Fernandez-Reyes, and provides a more practical
...
The book is directed to students of any computer science major,
...
However, for those of you who just need a quick reminder, we have included a
...
We are using this mix of languages to cover two different paradigms (object-oriented and functional
programming) and two different type systems (languages dynamically and
statically typed), shown in Table 1.1.

<!-- I like the image more, but I have to keep track of the table number -->
<!-- ![](../img/Table-Languages.png) -->

<!-- TODO: Does not look good on epub -->
```{r echo=FALSE, results='asis'}
library(knitr)
languages = matrix(c("Java", "Python", "Haskell", "Clojure"),
                   nrow=2,
                   ncol=2,
                   byrow = TRUE)
dimnames(languages) = list(
  c("Object-oriented", "Functional"),
  c("Static", "Dynamic")
)
kable(languages,
      caption="Language relation between paradigm and type system")
```

**Why multiple languages?**

Someone (who I don't remember) once said:

> "Choose the right tool for the right job."

Well, choosing a language is an important design decision as languages are tied
to a paradigm and a type system, and these cannot be changed. If you are a consultant
(I have been one for a few years) and you are given a new project, you should
try to avoid to go to your go-to language and think twice the requirements of the
project and the advantages and disadvantages that each language (paradigm and type system)
bring to the table. As usual, the more languages you know the better, since you
have more freedom to get a good start with the main features of your chosen language.
<!--   -->
...
This book does not provide an absolute truth nor it tries to do so, it merely
**show that the combination of different paradigms and type systems have inherent
benefits and drawbacks**. You have to work the examples in the book to find your truth.

## Organisation

The book is organised in 6 parts. The first part contains chapters 1 and 2 and
...
Afterwards, pattern is explained in the context of its paradigm and type system,
